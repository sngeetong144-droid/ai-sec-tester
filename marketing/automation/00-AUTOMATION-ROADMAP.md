...
> **Pricing (single source, `lib/payment-links.ts`):** $47 Normal / $197 Advanced — two distinct FastPayDirect links.
...
|---|---|---|---|---|---|---|
| 1 | **Daily "open requests aging" operator digest** (S4) | Vercel daily cron + Resend (internal email) | Hoping the real-time `sendNewRequestAlert` was seen; manually re-checking the queue | **S** (~1h) | **NOW** — internal email only | A silently-dropped alert = an aged-out request = a lost $47–$197 sale. Cheapest reliability backstop in the whole list. |
...
| G7 | **30-day PAID re-scan invite** (S5) | Daily-cron pass over completed scans (~day 25) + Resend; needs `completed_at` + a "rescan invited" guard flag | Nothing today. NOTE: this was scoped when a free re-scan was believed to be an entitlement. Ruling R-15 retired the Enterprise tier and NO tier includes a free re-scan — a re-scan is a NEW PAID SCAN. Rescope G7 as a paid re-scan invite or drop it | **S–M** | **Outbound send (light).** Creator approves the copy once. Also needs a delivered report to exist (G1). This SHALL NOT offer a free re-scan; the invite must sell a new scan. | Retention, plus a natural Normal → Advanced upgrade prompt. Lands on the same clock as the 30-day signed-URL expiry — one reminder covers both. |
