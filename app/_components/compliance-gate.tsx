...
const OFAC_BLOCKED = new Set([
  "CU","IR","KP","RU","SY","BY","VE","MM","SD","SS","YE","ZW","AF","LY","ML","NI","SO",
]);
...
export function ComplianceGate({ href, children, className }: ComplianceGateProps) {
  ...
  const isBlocked = country !== "" && OFAC_BLOCKED.has(country);
  const allChecked = checks.ofac && checks.authorized && checks.understood;
  const canProceed = allChecked && country !== "" && !isBlocked;
